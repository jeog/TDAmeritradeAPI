...
Test_QuotesGetter(struct Credentials* creds)
{
    ...
    const char* symbols_in[] = {"SPY", "QQQ", NULL};
    const char* symbols_in2[] = {"EEM", "QQQ", "GOOG", NULL};
    char **symbols_out;

    if( (err = QuotesGetter_Create(creds, symbols_in, &qsg)) )
    ...
    if( (err = QuotesGetter_SetSymbols(&qsg, symbols_in2)) )
