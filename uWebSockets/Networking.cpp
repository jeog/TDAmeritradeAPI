...
struct Init {
    Init() { /* NOTE - this init is redundant w/ libcurl's but leave it
                       to avoid any static init ordinality issues - J Ogden  */
        SSL_library_init();
    }
...
Context createContext(std::string certChainFileName, std::string keyFileName, std::string keyFilePassword)
{
    ...
    // SSLV12_server_method deprecated - J Ogden
    //Context context(SSL_CTX_new(TLS_server_method()));
    if (!context.context) {
        return nullptr;
    }
